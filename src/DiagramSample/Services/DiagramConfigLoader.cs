...
public class DiagramConfigLoader {
	...
	private void ValidateConfig (DiagramConfig config) {
		...
		foreach (var n in EnumerateAllNodes(config)) {
			if (string.IsNullOrWhiteSpace(n.Id)) {
				throw new Exception("Node missing id");
			}

			if (!ids.Add(n.Id!)) {
				throw new Exception($"Duplicate node id: {n.Id}");
			}
		}
		...
		foreach (var l in allLinks) {
			if (string.IsNullOrWhiteSpace(l.Id)) {
				throw new Exception("Link missing id");
			}

			if (string.IsNullOrWhiteSpace(l.From) || !ids.Contains(l.From)) {
				throw new Exception($"Link '{l.Id}' references unknown from '{l.From}'");
			}

			if (string.IsNullOrWhiteSpace(l.To) || !ids.Contains(l.To)) {
				throw new Exception($"Link '{l.Id}' references unknown to '{l.To}'");
			}

			foreach (var v in l.Via) {
				if (string.IsNullOrWhiteSpace(v) || !ids.Contains(v)) {
					throw new Exception($"Link '{l.Id}' references unknown via '{v}'");
				}
			}
		}

		// Validate business process steps (only grouped processes are allowed)
		var linkIds = new HashSet<string>(allLinks.Select(x => x.Id), StringComparer.OrdinalIgnoreCase);

		var allGroupedProcesses = config.BusinessProcessGroups
			.SelectMany(g => g.Processes ?? Enumerable.Empty<BusinessProcess>())
			.ToList();

		foreach (var bp in allGroupedProcesses) {
			if (string.IsNullOrWhiteSpace(bp.Id)) {
				throw new Exception("BusinessProcess missing id");
			}

			foreach (var s in bp.Steps) {
				if (!string.IsNullOrEmpty(s.Link)) {
					if (!linkIds.Contains(s.Link)) {
						throw new Exception($"BusinessProcess '{bp.Id}' references unknown link '{s.Link}'");
					}
				}
			}
		}
	...
	public object GetProcesses () {
		...
		// Build grouped representation
		var groups = new List<object>();

		foreach (var g in config.BusinessProcessGroups) {
			var procs = g.Processes
				.Where(bp => !string.IsNullOrWhiteSpace(bp.DisplayName))
				.Select(bp => new {
						id = bp.Id,
			...
			if (procs.Any()) {
				groups.Add(
					new {
						...
						processes = procs,
						hiddenByDefault = g.HiddenByDefault
					}
				);
			}
		}

		// Keep return shape compatible with the frontend: provide groups and an empty ungrouped list
		return new { groups = groups, processes = new List<object>() };
	}
}
